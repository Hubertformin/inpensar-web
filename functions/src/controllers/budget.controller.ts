...
export const createBudgetController = createController(async (req, res) => {
    ...
    if (conflict_budget) {
        const existing_categories = conflict_budget.categories.filter((c) => (budgetData.categories as any[]).findIndex((id: string) => id == c._id?.toString()) > -1).map((c) => c.name);
...
export const getCurrentUserBudgetsController = createController(async (req, res) => {
    ...
    const budgets = await Budget.find({ owner: req.$currentUser$?._id }).skip(startIndex).limit(limit).populate("categories").exec();
...
export const getUserBudgetByIdController = createController(async (req, res) => {
    ...
    if (!budget) {
        throw CustomError("Budget not found").status(404);
    }
...
export const updateUserBudgetController = createController(async (req, res) => {
    ...
    const conflict_budget: BudgetDocument = (await Budget.findOne({ _id: { $ne: req.params.id }, categories: { $in: budgetData.categories } })
        .populate("categories", "_id name")
        .exec()) as BudgetDocument;
